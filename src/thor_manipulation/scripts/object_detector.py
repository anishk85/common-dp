#!/usr/bin/env python3
# filepath: /home/anish/dp_ws/Thor-ROS/ws_thor/src/thor_manipulation/scripts/object_detector.py
...
class ObjectDetector(Node):
    def __init__(self):
        ...
        # Publishers
        self.detection_pub = self.create_publisher(PoseStamped, '/detected_objects', 10)
        self.status_pub = self.create_publisher(String, '/detection_status', 10)
        
        # Subscribers
        self.trigger_sub = self.create_subscription(
            String, '/thor_arm/pick_place_command', self.trigger_callback, 10)
...
if __name__ == '__main__':
    main()
        # Known object positions (based on spawn locations)
        self.known_objects = {
            'red_can': [0.3, 0.0, 0.05],
            'green_box': [0.4, 0.1, 0.025], 
            'blue_sphere': [0.35, -0.1, 0.03],
            'can': [0.3, 0.0, 0.05],  # Alternative names
            'box': [0.4, 0.1, 0.025],
            'sphere': [0.35, -0.1, 0.03]
        }
        ...
        self.get_logger().info("📷 Object Detector initialized (using known positions)")

    def spawn_status_callback(self, msg):
        """Track what objects are spawned - IMPROVED PARSING"""
        status = msg.data.lower()
        
        self.get_logger().info(f"🔍 Received spawn status: '{msg.data}'")
        
        # Check for various spawn message formats
        if "red_can" in status or "can" in status:
            self.last_spawned = 'red_can'
            if 'red_can' not in self.spawned_objects:
                self.spawned_objects.append('red_can')
                self.get_logger().info("📦 Added red_can to spawned objects")
                
        elif "green_box" in status or "box" in status:
            self.last_spawned = 'green_box'
            if 'green_box' not in self.spawned_objects:
                self.spawned_objects.append('green_box')
                self.get_logger().info("📦 Added green_box to spawned objects")
                
        elif "blue_sphere" in status or "sphere" in status:
            self.last_spawned = 'blue_sphere'
            if 'blue_sphere' not in self.spawned_objects:
                self.spawned_objects.append('blue_sphere')
                self.get_logger().info("📦 Added blue_sphere to spawned objects")
        
        self.get_logger().info(f"📦 Current spawned objects: {self.spawned_objects}")

    def trigger_callback(self, msg):
        """Trigger object detection"""
        if msg.data.upper() == "DETECT":
            self.detect_objects()

    def detect_objects(self):
        """Detect objects using known positions"""
        self.get_logger().info(f"🔍 Detection triggered. Spawned objects: {self.spawned_objects}")
        
        if not self.spawned_objects:
            self.get_logger().warn("⚠️ No objects spawned yet. Spawn an object first!")
            
            # Publish failure status
            status_msg = String()
            status_msg.data = "NO_OBJECTS_SPAWNED"
            self.status_pub.publish(status_msg)
            return
        
        # Detect the most recently spawned object
        target_object = self.last_spawned or self.spawned_objects[-1]
        
        self.get_logger().info(f"🎯 Targeting object: {target_object}")
        
        if target_object in self.known_objects:
            pos = self.known_objects[target_object]
            ...
            self.get_logger().info(f"📍 Object detected: {target_object} at "
                                 f"({pos[0]}, {pos[1]}, {pos[2]})")
            
            # Publish status
            status_msg = String()
            status_msg.data = f"DETECTED_{target_object.upper()}"
            self.status_pub.publish(status_msg)
        else:
            self.get_logger().error(f"❌ Unknown object type: {target_object}")
            self.get_logger().info(f"Available objects: {list(self.known_objects.keys())}")
